...
% Irreducible representations of a direct product of groups.
    ...
    methods
        ...
        function [a, I] = juliasort(a, varargin)
            [I, a.charges{:}] = simulsort_reverse(a.charges{:}, varargin{:});
        end
        ...
        function [a, I] = juliasortrows(a, col, direction)
            arguments
                a
                col (1,:) double = 1:size(a, 2)
                direction = 'ascend'
            ...
            newcol = reshape(flipud(col + (((1:length(a.charges)).' - 1) .* size(a, 2))), 1, []);
            [I, a.charges{:}] = simulsortrows(a.charges{:}, ...
                'Col', newcol, ...
                'Direction', direction);
        end
