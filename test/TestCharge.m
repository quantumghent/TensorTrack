classdef TestCharge < matlab.unittest.TestCase
    ...
    properties (TestParameter)
        smallset = struct( ...
            ...
            'SU2', SU2(1:4), ...
            'A4', A4(1:4), ...
            'SU3', SUN([1 0 0], [2 0 0], [3 0 0]), ...
            'Z2xU1', ProductCharge(Z2(0, 0, 0, 1, 1, 1), U1(-1, 0, 1, -1, 0, 1)), ...
            'U1xSU2', ProductCharge(U1(-1, -1, 0, 0, 1, 1), SU2(1, 2, 1, 2, 1 ,2))...
            )
    end
    ...
    methods (Test)
                        ...
                        for e = a * b
                            ...
                            for f = b * c
                                ...
                                for d = intersect(e * c, a * f)
                                    ...
                                    actual = contract(F1, [1 6 4 -4], ...
                                        F2, [2 3 6 -3], F3, [1 2 5 -1], F4, [5 3 4 -2]);
                                    expected = Fsymbol(a, b, c, d, e, f) * qdim(d);
        ...
        function Funitary(testCase, smallset)
            ...
            for a = smallset, for b = smallset, for c = smallset
                        for d = prod([a, b, c])
                            ...
                            testCase.assertTrue(isapprox(Fmat' * Fmat, eye(size(Fmat)), ...
                                'AbsTol', testCase.tol));
                            testCase.assertTrue(isapprox(Fmat * Fmat', eye(size(Fmat)), ...
                                'AbsTol', testCase.tol));
                        end
        ...
        function Rmove(testCase, smallset)
            ...
            for a = smallset
                for b = smallset
                    for c = a * b
                        expected = fusiontensor(a, b, c);
                        actual = contract(fusiontensor(b, a, c), [-2 -1 -3 1], ...
                            Rsymbol(a, b, c), [-4 1]) * ...
                            (-2 * double(parity(a) & parity(b)) + 1);
                        testCase.verifyTrue(isapprox(actual, expected, ...
                            'AbsTol', testCase.tol, 'RelTol', testCase.tol));
                    end
                end
            end
